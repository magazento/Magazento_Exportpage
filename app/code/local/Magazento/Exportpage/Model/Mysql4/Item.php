...
class Magazento_Exportpage_Model_Mysql4_Item extends Mage_Core_Model_Mysql4_Abstract {

    protected function _construct() {
        $this->_init('exportpage/item', 'item_id');
    }
   ...
   protected function _beforeSave(Mage_Core_Model_Abstract $object) {
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object) {
        ...
        $this->_getWriteAdapter()->delete($this->getTable('exportpage/item_store'), $condition);
        if ($object->getData('in_related')) $this->_getWriteAdapter()->delete($this->getTable('exportpage/item_related'), $condition);
...
//        exit();        
        ...
        foreach ((array) $relateds as $related) {
            ...
            $this->_getWriteAdapter()->insert($this->getTable('exportpage/item_related'), $relatedArray);
        }        
        ...
        foreach ((array) $object->getData('stores') as $store) {
            ...
            $this->_getWriteAdapter()->insert($this->getTable('exportpage/item_store'), $storeArray);
        }
    ...
    protected function _afterLoad(Mage_Core_Model_Abstract $object) {
        ...
        $select = $this->_getReadAdapter()->select()
                        ->from($this->getTable('exportpage/item_store'))
        ...
        //RELATED
        $select = $this->_getReadAdapter()->select()
                        ->from($this->getTable('exportpage/item_related'))
                        ->where('item_id = ?', $object->getId());
        if ($data = $this->_getReadAdapter()->fetchAll($select)) {
            $relatedArray = array();
            foreach ($data as $row) {
    ...
    protected function _beforeDelete(Mage_Core_Model_Abstract $object) {
        ...
        $adapter->delete($this->getTable('exportpage/item_store'), 'item_id=' . $object->getId());
        $adapter->delete($this->getTable('exportpage/item_related'), 'item_id=' . $object->getId());
    }
